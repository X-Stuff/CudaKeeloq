...
#include "host/types/bruteforce_filters.cpp"
...
{
	__global__ void CUDA_GenerateDecryptorsFiltered(KernelInput::TCudaPtr input, KernelResult::TCudaPtr results)
	{
		...
		CUDA_FOR_THREAD_ID(ctx, write_index, num_decryptors)
		{
			...
			do
			{
				if (block_size == 0)
				{
					...
					//
					man_block_begin = atomicAdd(&input->generator.next.man, block_size);
	...
	{
		CUDA_GenerateDecryptorsFiltered<<<blocks, threads>>>(input, results);
	}
}
...
namespace Tests
{
	__global__ void Kernel_RunFiltersTests_device(BruteforceFilters::Test::Inputs* tests, uint8_t num)
	{
		for (int i = 0; i < num; ++i)
		{
	...
	void Kernel_RunBruteforceFiltersTests(BruteforceFilters::Test::Inputs* tests, uint8_t num, uint32_t blocks, uint32_t threads)
	{
		Kernel_RunFiltersTests_device<<<blocks,threads>>>(tests, num);
	}
}