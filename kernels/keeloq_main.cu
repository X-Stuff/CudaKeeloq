...
__device__ uint8_t keeloq_decryption_run(const CudaContext& ctx, KernelInput& input)
{
    ...
    CUDA_FOR_THREAD_ID(ctx, decryptor_index, decryptors.num)
    {
        ...
        for (uint32_t enc_index = 0; enc_index < encrypted.num; enc_index++)
        {
            ...
            result.results = keeloq_decrypt(result.ota, result.man, result.seed, input.learning_types);
        }
...
__device__ __host__ SingleResult::DecryptedArray keeloq_decrypt(uint64_t ota, uint64_t man, uint32_t seed, const KeeloqLearningType::Type type_mask[])
{
    ...
    if (IsAllLearningTypesEnabled(type_mask)) // force all decryption learnings
    {
        // compiler should optimize out the ifs in that case since condition became `(true || XXXX`
        return keeloq_decrypt_all<true>(hop, fix, man, seed, type_mask);
    }
    else
    {
        return keeloq_decrypt_all<false>(hop, fix, man, seed, type_mask);
    }
}
