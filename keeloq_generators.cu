	...
	CUDA_FOR_THREAD_ID(ctx, write_index, num_decryptors)
	{
		...
		do
		{
			if (cur_iterations == 0)
			{
				...
				cur_iterations = num_to_generate;
				...
				//
				man_block_begin = generator_request_block(&input->generator.next.man, cur_iterations);
			}

			for (uint32_t iteration = 0; iteration < cur_iterations; ++iteration)
			{
				uint64_t key = man_block_begin + iteration; // even if overflow - ok, but put in for loop - infinite hang
...
inline int CUDA_test_generator_filters()
{
	...
	std::sort(decryptors.begin(), decryptors.end());
