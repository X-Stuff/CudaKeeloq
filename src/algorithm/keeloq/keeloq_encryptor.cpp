...
EncParcel Encryptor::click(InputsTransform inTransform, KeeloqLearning::LearningType ltype, KeeloqLearning::Modifier::Algo amod)
{
    const auto cpu_encrypted = cpuEncrypt(inTransform, ltype, amod);
    assert(gpuEncrypt(inTransform, ltype, amod) == cpu_encrypted && "GPU and CPU encryption results do not match");

    const uint64_t detpyrcne = ((uint64_t)fixed() << 32) | cpu_encrypted;
    const auto ota = misc::rev_bits(detpyrcne);

    const auto cpu_decrypted = cpuDecrypt(ota, inTransform, ltype, amod);

    assert(cpu_decrypted == gpuDecrypt(ota, inTransform, ltype, amod) && "GPU and CPU decryption results do not match");
...
uint64_t Encryptor::man(InputsTransform inTransform, KeeloqLearning::LearningType ltype, KeeloqLearning::Modifier::Algo amod) const
{
...
uint32_t Encryptor::gpuDecrypt(uint64_t enc, InputsTransform inTransform, KeeloqLearning::LearningType ltype, KeeloqLearning::Modifier::Algo amod) const
{
    const uint8_t resIndex = KeeloqLearning::DecryptedResults::getIndex(ltype, amod);
