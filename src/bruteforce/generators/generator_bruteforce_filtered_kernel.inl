...
__global__ void DEFINE_GENERATOR_KERNEL(GeneratorBruteforceFiltered, IKeeloqKernelInputBase::Ptr input, KernelResult::TCudaPtr resuls)
{
    ...
    const size_t num_decryptors = decryptors.num;
    ...
    const uint8_t at_least_one = num_decryptors >= ctx.thread_id;

    // if we have to generate 75 keys with 64 threads, 11 threads should do +1 key generation
    const size_t non_aligned = num_decryptors % ctx.thread_max;
    const uint8_t additional_this_thread = non_aligned > 0 && non_aligned > ctx.thread_id;
    ...
    CUDA_FOR_THREAD_ID(ctx, write_index, num_decryptors)
    {
        ...
        do
        {
            if (block_size == 0)
            {
                ...
                //
                man_block_begin = RequestNewBlock(&filters.sync_key, block_size);
