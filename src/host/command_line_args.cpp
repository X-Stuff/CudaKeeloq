...
namespace
{
    ...
    inline void parse_dictionary_mode(CommandLineArgs& args, cxxopts::ParseResult& result, AppVerbosity verbosity)
    {
        ...
        if (result[ARG_WORDDICT].count() > 0)
        {
            ...
            {
                args.brute_configs.push_back(BruteforceConfig::GetDictionary(std::move(decryptors), args.inputsTransform));
            }
        }
    ...
    inline void parse_bruteforce_mode(CommandLineArgs& args, cxxopts::ParseResult& result, AppVerbosity verbosity)
    {
        ...
        args.brute_configs.push_back(BruteforceConfig::GetBruteforce(first_decryptor, args.inputsTransform, count_key));
    }
    ...
    inline void parse_bruteforce_filtered_mode(CommandLineArgs& args, cxxopts::ParseResult& result, AppVerbosity verbosity)
    {
        ...
        args.brute_configs.push_back(BruteforceConfig::GetBruteforce(first_decryptor, args.inputsTransform, count_key, filters));
    }
    ...
    inline void parse_alphabet_mode(CommandLineArgs& args, cxxopts::ParseResult& result, AppVerbosity verbosity)
    {
        ...
        {
            args.brute_configs.push_back(BruteforceConfig::GetAlphabet(first_decryptor, args.inputsTransform, alphabet, count_key));
        }
    }
    ...
    inline void parse_pattern_mode(CommandLineArgs& args, cxxopts::ParseResult& result, AppVerbosity verbosity)
    {
        ...
        for (const auto& pattern_arg : args_pattern)
        {
            ...
            args.brute_configs.push_back(BruteforceConfig::GetPattern(first_decryptor, args.inputsTransform, BruteforcePattern(std::move(result), pattern_arg), count_key));
        }
    }
...
CommandLineArgs CommandLineArgs::parse(int argc, const char** argv, AppVerbosity verbosity)
{
    ...
    // CUDA setup
    args.cuda_blocks = result[ARG_BLOCKS].as<uint16_t>();
    args.cuda_threads = result[ARG_THREADS].as<uint16_t>();
    ...
    // Modifications
    {
        ...
        if (result[ARG_CHECK_REVKEYS].as<bool>())
        {
    ...
    {
        const auto learning_type_names = result[ARG_LTYPE].as<std::vector<std::string>>();
    ...
    const auto learningMatrix = args.getLearningMatrix();
    for (auto& config : args.brute_configs)
    {
        config.setLearningMatrix(learningMatrix);
    }
